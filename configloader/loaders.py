...
class ConfigLoader:
    ...
    def load_config(self, config_file: str):
        ...
        for active_section in self.active_sections:
            if active_section in config:
                try:
                    app_config = self.active_sections[active_section](**config[active_section])
    ...
    def load_configs(self):
        ...
        if len(missing_configs) > 0:
            raise Exception('These config sections are missing in config files: ' + str(missing_configs))
